...
impl TryFrom<youtube::interfaces::video::VideoResourse> for Song {
    ...
    fn try_from(value: youtube::interfaces::video::VideoResourse) -> Result<Self, Self::Error> {
        ...
        let url = match Url::parse(&url) {
            Ok(url) => url,
            Err(why) => {
                return Err(AudiophileError { location: "Song::from<VideoResourse>", message: "Cannot parse YouTube URL while constructing the song", cause: Some(Box::new(why))});
            }
        };
        ...
        let thumbnail = song_snippet.thumbnails.get(&youtube::interfaces::video::VideoResourseThumbnailType::Default);
